...
export default function Portfolio() {
  ...
  return (
    <main className="min-h-screen bg-[#030303] text-neutral-100 selection:bg-white/10 selection:text-white font-sans relative">
      ...
      <AnimatePresence>
        {showNav && (
          <motion.div
            ...
            className="fixed top-8 z-[100] flex items-center gap-6 px-5 py-2.5 md:px-7 md:py-3.5 bg-neutral-900/40 backdrop-blur-2xl border border-white/15 rounded-full shadow-[0_20px_50px_rgba(0,0,0,0.5)] w-max
                       left-1/2 -translate-x-1/2 md:left-8 md:translate-x-0"
          >
            <button 
              onClick={scrollToTop}
              className="font-bold tracking-tighter lowercase text-white hover:opacity-70 transition-opacity whitespace-nowrap"
            >
      ...
      {/* FOOTER */}
      <footer className="py-12 md:py-16 text-center border-t border-white/[0.02] px-6">
        <div className="flex flex-wrap justify-center items-center gap-x-2.5 gap-y-1 text-[10px] md:text-xs text-neutral-800 uppercase tracking-[0.3em] font-medium">
          <span>&copy; {new Date().getFullYear()}</span>
          <span className="text-neutral-700">albert bastl</span>
          <span className="text-neutral-900 hidden md:inline">&mdash;</span>
          <span className="text-neutral-900">built with next.js</span>
        </div>
      </footer>
    </main>
  );
